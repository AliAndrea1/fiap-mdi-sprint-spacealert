        ...
        <View style={styles.grafico}>
          ...
          <View style={styles.graficoArea}>
            {[0, 1, 2, 3, 4, 5].map((n) => (
              <View
                key={n}
                style={[styles.linhaHorizontal, { bottom: (n / GRAFICO_MAX) * GRAFICO_ALTURA }]}
              />
            ))}
            {barras.map(item => (
...
const styles = StyleSheet.create({
  ...
  riscoCard: {
    ...
    borderRadius: 12, borderWidth: 2, alignItems: 'center',
  },
  riscoLabel: { fontSize: 12, color: Colors.textSecondary, letterSpacing: 2 },
  riscoValue: { fontSize: 32, fontWeight: 'bold', marginTop: 8 },
  riscoSub: { fontSize: 12, color: Colors.textSecondary, marginTop: 4 },
  cardsRow: { flexDirection: 'row', paddingHorizontal: 16, gap: 8 },
  card: {
    flex: 1, backgroundColor: Colors.card, borderRadius: 10,
    padding: 16, borderLeftWidth: 4, alignItems: 'center',
  },
  cardNumber: { fontSize: 28, fontWeight: 'bold', color: Colors.text },
  cardLabel: { fontSize: 11, color: Colors.textSecondary, marginTop: 4 },
  chartContainer: {
  ...
  eixoYArea: {
    width: 20, height: GRAFICO_ALTURA,
    justifyContent: 'space-between',
    marginRight: 6, marginBottom: LABEL_ALTURA,
  },
  eixoYLabel: { fontSize: 10, color: Colors.textSecondary, textAlign: 'right' },
  graficoArea: {
    flex: 1,
    height: GRAFICO_ALTURA + LABEL_ALTURA,
    flexDirection: 'row',
  ...
  barra: { width: '45%', borderRadius: 4, borderWidth: 1 },
  barraLabel: {
    fontSize: 9, color: Colors.textSecondary,
    textAlign: 'center', marginTop: 4, height: LABEL_ALTURA,
  },
  section: { margin: 16 },
  alertItem: {
  ...
  alertInfo: { flex: 1 },
  alertTitulo: { fontSize: 13, fontWeight: '600', color: Colors.text },
  alertRegiao: { fontSize: 11, color: Colors.textSecondary, marginTop: 2 },
  alertNivel: { fontSize: 11, fontWeight: 'bold' },
  fonte: { textAlign: 'center', color: Colors.textSecondary, fontSize: 11, marginBottom: 32 },
});